    ...
    def bfs():
        ...
        while len(queue):
            ...
            for i in range(0, 4):
                ...
                if (0 <= nx and nx < field_x_length and 0 <= ny and ny < field_y_length and maze[nx][ny] != 1 and distance[nx][ny] == INF):
...
def makeroot(sx,sy,gx,gy,maze,distance):
    ...
    while x!=sx or y!=sy:
        for i in range(0, 4):
            if count%50==0:
                im = Image.open(os.path.join('static', 'img_maze','pillow_imagedraw.jpg'))
...
def create_graph_string(G):
    ...
    for i in range(len(str1)):
        graph_str+=str(list(G.nodes)[i][0])+" "+str(list(G.nodes)[i][1])+"\n"

    graph_str+=str(len(G.edges))+"\n"

    for i in range(len(str2)):
        graph_str+=str(list(G.edges)[i][0][0])+" "+str(list(G.edges)[i][0][1])+" "+str(list(G.edges)[i][1][0])+" "+str(list(G.edges)[i][1][1])+"\n"
...
def index2():
    if request.method == 'POST':
        ...
        if value == "1":
            ...
            #to confirm2
            return render_template('form2.html',title="Maze",name=name,pic=pic1)
...
def show_img():
    if request.method == 'POST':
        ...
        return render_template("confirm.html",title="Maze",img_url=img_url22,filename=filename,sss=sss)
